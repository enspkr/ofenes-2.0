 ...
 */
export function useWebRTC({ messages, sendDirect, username, isConnected }: UseWebRTCOptions) {
    ...
    // ─── Track WebSocket disconnects while in call ───
    useEffect(() => {
            ...
            for (const msg of newMessages) {
                try {
                    if (msg.type === 'user_list') {
                        ...
                        // Auto-rejoin: after a WebSocket reconnect, re-offer to peers we lost
                        if (wasDisconnectedRef.current && isInCallRef.current) {
                            wasDisconnectedRef.current = false
                            const missingPeers = users.filter(
                                (u) => u !== usernameRef.current && !peersRef.current.has(u)
                            )
                            for (const remoteUser of missingPeers) {
                                const pc = createPeerConnection(remoteUser)
                                try {
                                    const offer = await pc.createOffer()
                                    await pc.setLocalDescription(offer)
                                    sendSignal('offer', remoteUser, { sdp: pc.localDescription?.toJSON() })
                                    console.log(`[webrtc] auto-rejoin: sent offer to ${remoteUser}`)
                                } catch (err) {
                                    console.error(`[webrtc] auto-rejoin offer failed for ${remoteUser}:`, err)
                                }
                            }
                        }

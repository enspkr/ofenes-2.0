...
func New(application *app.App) http.Handler {
	...
	// Game — YouTube proxy (auth required so the API key stays server-side)
	mux.Handle("GET /api/game/search/artist", authMw(http.HandlerFunc(h.SearchArtist)))
	mux.Handle("GET /api/game/artist/tracks", authMw(http.HandlerFunc(h.GetArtistTracks)))
	mux.Handle("GET /api/game/genres", authMw(http.HandlerFunc(h.GetGenres)))
	mux.Handle("GET /api/game/genre/tracks", authMw(http.HandlerFunc(h.GetGenreTracks)))

...
// roomBroadcastMsg is used by goroutines (e.g. game timers) to safely send
// a message to a room without touching h.clients directly.
...
type Hub struct {
	...
	roomBroadcast chan roomBroadcastMsg // for async game events from goroutines
...
func NewHub(messageRepo repository.MessageRepository, gameManager *game.Manager) *Hub {
	return &Hub{
		Broadcast:      make(chan []byte),
		Register:       make(chan *Client),
		Unregister:     make(chan *Client),
		roomBroadcast:  make(chan roomBroadcastMsg, 128),
...
func (h *Hub) Run() {
	for {
		...
		case rb := <-h.roomBroadcast:
			h.broadcastToRoom(rb.RoomID, rb.Data)
		}
	}
}
...
func (h *Hub) addClient(client *Client) {
	...
	// Push current game state to the new client if a game is active
	if session, ok := h.gameManager.Get(room); ok {
		session.Lock()
		snap := session.Snapshot()
		session.Unlock()
		if data := h.buildGameMessage(map[string]interface{}{
...
func (h *Hub) routeMessage(raw []byte) {
	...
	case models.MsgTypeGame:
		h.handleGameMessage(room, msg)
